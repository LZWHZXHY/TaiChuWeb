      ...
      >
        ...
        <div class="mc-actions-wrapper">
          <div class="utility-row">
            <button class="icon-btn" title="查看预览" @click="$router.push(`/activity/${item.id}`)">👁️</button>
...
import JointProposalModal from '@/CreateCenterComponents/components/ActivityProposalModal.vue'
...
// --- 1. 数据调取 (已更新为 Activity 路径) ---
const fetchMyJoints = async () => {
  loading.value = true
  try {
    const res = await apiClient.get('Activity/my-hosted')
...
const handleEdit = async (item) => {
  loading.value = true
  try {
    const res = await apiClient.get(`Activity/${item.id}`)
...
// --- 3. 指令下达 (对接到 Activity 或 AdminManager) ---
const handleStatusChange = async (item, actionType) => {
  const msg = actionType === 'close' ? '确定要封存此行动吗？' : '确定要重启该行动吗？'
  if (!confirm(msg)) return
  try {
    // 这里如果你的后端重构后该功能归属管理员控制，改为 Admin/ActivityManager/${actionType}
    // 如果发起人仍能操作，则使用 Activity/moderation/${actionType}
    await apiClient.post(`Activity/moderation/${actionType}`, { id: item.id })
    fetchMyJoints()
...
const handleDelete = async (item) => {
  ...
  try {
    await apiClient.delete(`Activity/${item.id}`)
    fetchMyJoints()
  } catch (e) { alert('删除受阻') }
}
...
const fetchApplicants = async (opId) => {
  ...
  try {
    const res = await apiClient.get(`Activity/${opId}/applications`)
...
const audit = async (app, isPass) => {
  try {
    await apiClient.post('Activity/application/audit', { appId: app.AppId, pass: isPass })
...
const toggleUserFiles = async (app) => {
  ...
  if (!userSubmissions.value[app.UserId]) {
    try {
      const res = await apiClient.get(`Activity/${selectedOp.value.id}/submissions/${app.UserId}`)
...
const handleDownload = async (file) => {
  try {
    const res = await apiClient.get(`Activity/submission/download/${file.Id}`)
...
const typeLabelCn = (t) => {
  // 根据咱们新分配的 Type ID 显示中文标签
  if (t >= 1 && t <= 2) return '动画协作'
  if (t >= 3 && t <= 5) return '绘圈企划'
  if (t >= 6 && t <= 9) return '跑团招募'
  if (t >= 10) return '世界观共创'
  return '常规行动'
}
...
.manage-card { background: #fff; border: 2px solid #111; padding: 25px; position: relative; display: flex; flex-direction: column; min-height: 240px; transition: 0.3s; box-shadow: 6px 6px 0 #111; }
...
.ts-item .label { font-size: 0.65rem; font-weight: 900; color: #999; margin-right: 12px; }
...
.terminal-nav button { flex: 1; border: none; padding: 14px; font-weight: 900; font-size: 0.75rem; background: #eee; cursor: pointer; color: #888; }
.terminal-nav button.active { background: #fff; color: #111; }

.console-body { flex: 1; padding: 25px; background: #fff; overflow-y: auto; }
.t-row { display: flex; align-items: center; justify-content: space-between; padding: 15px; border: 1px solid #eee; margin-bottom: 12px; background: #fff; }
.t-avatar { width: 45px; height: 45px; border: 2px solid #111; }
.act-btn { border: 2px solid #111; padding: 6px 15px; font-weight: 900; font-size: 0.7rem; cursor: pointer; }
...
.file-card { background: #fff; border: 1px solid #ddd; padding: 12px; display: flex; gap: 12px; align-items: center; }

.loading-txt { padding: 40px; text-align: center; color: #999; font-family: monospace; }
.empty-txt { padding: 60px; text-align: center; color: #bbb; font-weight: 900; }
