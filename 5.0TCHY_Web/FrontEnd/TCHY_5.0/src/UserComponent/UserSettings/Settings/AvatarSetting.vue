    ...
    <div class="bottom-section">
      ...
      <transition name="slide-up">
        <div v-if="tempSelectedId && activeTab === 'default'" class="floating-bar">
          ...
          <button class="apply-btn" @click="confirmDefaultSelection">应用 // APPLY</button>
        </div>
      </transition>
    </div>
  </div>
...
// --- 状态与逻辑 ---
const currentAvatar = ref('https://img.bianyuzhou.com/uploads/默认头像/默认头像2.png'); 
...
const defaultAvatars = [
  { id: 1, name: '预设头像_01', url: 'https://img.bianyuzhou.com/uploads/默认头像/默认头像1.png' },
  { id: 2, name: '预设头像_02', url: 'https://img.bianyuzhou.com/uploads/默认头像/默认头像2.png' },
  { id: 3, name: '预设头像_03', url: 'https://img.bianyuzhou.com/uploads/默认头像/默认头像3.png' },
  { id: 4, name: '预设头像_04', url: 'https://img.bianyuzhou.com/uploads/默认头像/默认头像4.png' },
];
...
const processFile = (file) => {
  ...
  if (!validTypes.includes(file.type)) {
    alert('不支持的文件格式 (仅限 JPG/PNG/GIF)');
    return;
  }

  // 校验大小
  if (file.size > 5 * 1024 * 1024) { 
    alert('文件过大: 5MB (MAX SIZE)'); 
    return; 
  }
...
const handleFileChange = (e) => {
  ...
  processFile(file);
  e.target.value = ''; // 重置 input，防止重复选择同一文件不触发 change
};
...
const handleDrop = (e) => {
  ...
  const file = e.dataTransfer.files[0]; // 获取拖拽的第一个文件
  processFile(file);
};
...
const confirmCrop = () => {
  ...
  canvas.width = size * scaleX; canvas.height = canvas.width;
  const ctx = canvas.getContext('2d');
  ctx.drawImage(sourceImgRef.value, (cropState.boxX - cropState.imgRenderX) * scaleX, (cropState.boxY - cropState.imgRenderY) * scaleX, canvas.width, canvas.height, 0, 0, canvas.width, canvas.height);
  currentAvatar.value = canvas.toDataURL('image/png');
  isEditing.value = false; tempSelectedId.value = null;
};
const preSelectDefaultAvatar = (item) => { tempSelectedId.value = item.id; tempSelectedName.value = item.name; tempSelectedUrl.value = item.url; };
const confirmDefaultSelection = () => { if (!tempSelectedId.value) return; currentAvatar.value = tempSelectedUrl.value; };
</script>
...
.upload-right.is-dragging {
  ...
  background-color: #e6f7ff; /* 或使用主色调淡色 */
  transform: scale(0.98); /* 微微缩小产生按压感 */
...
.upload-text h3 { margin: 0; font-size: 16px; font-weight: 800; font-family: 'Noto Sans SC', sans-serif; }
...
.corner-marker {
  position: absolute; width: 10px; height: 10px; border-color: #fff; border-style: solid;
}
...
.editor-header .step-label { font-size: 10px; color: #666; font-family: 'Noto Sans SC'; }
.editor-header h3 { margin: 5px 0; font-size: 14px; font-family: 'Noto Sans SC'; color: #fff; }

.editor-actions { display: flex; flex-direction: column; gap: 10px; }
.action-btn {
  height: 40px; border: none; cursor: pointer; font-family: 'Noto Sans SC'; font-weight: bold; font-size: 12px;
  transition: all 0.2s;
}
.confirm-btn { background: #fff; color: #000; border-radius: 4px; }
.confirm-btn:hover { background: #e6e6e6; }
.cancel-btn { background: #000000; color: #666; border-radius: 4px;}
...
.empty-state {
  ...
  color: #ccc; font-family: 'Roboto Mono';
}
...
.bar-info { display: flex; gap: 10px; font-family: 'Noto Sans SC'; font-size: 12px; }
...
.apply-btn {
  ...
  font-weight: 900; font-family: 'Noto Sans SC';
