...
public class EmailSender
{
    ...
    public async Task SendAsync(string to, string subject, string body, bool isHtml = false)
    {
        ...
        // 详细的配置检查
        _logger.LogInformation($"SMTP主机: {host}");
        _logger.LogInformation($"SMTP端口: {portStr}");
        _logger.LogInformation($"SMTP用户名: {username}");
        _logger.LogInformation($"SMTP密码长度: {password?.Length}");

        // 安全地显示密码前几位（避免空引用）
        if (!string.IsNullOrEmpty(password) && password.Length >= 3)
        {
            _logger.LogInformation($"SMTP密码前3位: {password.Substring(0, 3)}****");
        }
        else
        {
            _logger.LogWarning("SMTP密码为空或长度不足");
        }
